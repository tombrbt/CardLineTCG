...
type CardsResponse = {
  success: boolean;
  cards: any[];
  variantsIndex?: Record<string, string[]>;
  total?: number;
  totalPages?: number;
  page?: number;
  pageSize?: number;
...
export default function CardList() {
  ...
  const [sort, setSort] = useState<"code_asc" | "code_desc" | "price_low_asc" | "price_low_desc">("code_asc");
  ...
  const { data, isLoading, isError, error, isFetching } = useQuery<CardsResponse>({
    queryKey: ["cards", deferredSearch, color, type, rarity, family, page, setCode, sort],
    queryFn: async () => {
  ...
  type SetsResponse = {
    success: boolean;
    sets: { code: string; name_fr: string }[];
  };
  ...
  function getVariantsSet(card: any) {
    const key = `${card.setId}:${card.code}`;
  ...
  return (
    <div className="p-4 flex-1 container mx-auto px-4 py-8">
      <div className="relative flex-1 mb-4">
        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" className="lucide lucide-search absolute left-3 top-1/2 -translate-y-1/2 text-zinc-400" aria-hidden="true"><path d="m21 21-4.34-4.34"></path><circle cx="11" cy="11" r="8"></circle></svg>
        {/* INPUT */}
